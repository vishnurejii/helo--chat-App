...
getUsers()
  ...
  return (
    <div className={`bg-[#8185B2]/10 h-full p-3 rounded-r-xl overflow-y-auto text-white ${selectedUser ? "max-md:hidden": ''}`}>
       ...
       <div className='flex flex-col'>
        {filteredUsers.map((user,index)=>(
            <div 
              ...
              key={index} 
              className={`relative flex items-center gap-3 py-2 px-3 rounded cursor-pointer max-sm:text-sm ${selectedUser?._id === user._id && 'bg-[#282142]/50'}`}
            >
                <img 
